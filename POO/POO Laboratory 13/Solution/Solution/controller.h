	...
	public:
		...
		/*Creates and add a movie class to the repository attribute
			input:
				...
				newTrailer - link*/
		void add(char newTitle[30], char newGenre[30], char newTrailer[100], int newYear, int newLikes);
		...
		void update(char newTitle[30], char newGenre[30], char newTrailer[100], int newYear, int newLikes);
		...
		std::vector<Movie> getVectorGenre(std::string genre);

		/*Returns a refference to the vector class*/
		std::vector<Movie>& getVector();
		void removeByIndex(int index);
		void addWatchList(Movie someMovie);
		std::vector<Movie>& getWatchList();
		Movie findByName(std::string name);
};
